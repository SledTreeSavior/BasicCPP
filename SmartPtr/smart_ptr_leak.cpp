...
int main () {
  ...
  const bool useSmartPointers = true;  // *** change to switch between smart and built-in pointers ***
  
  const int mySize = 1000000;
  const int numIterations = 700000;
  int i = 0;
  
  cout << "Demo of memory leak behavior when using smart pointers versus built-in pointers.\n";
  cout << "***** For this run, we are using " << ((useSmartPointers) ? "smart" : "built-in") << " pointers!\n\n";
  ...
  cout << "Allocation loop exited after " << i << " allocations (should be " << numIterations << " if no memory leaks!)" << endl;
